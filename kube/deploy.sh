...
sudo docker pull mubaraklouis/missservice:0.0.12
...
if [ "$debug_mode" = "y" ]; then
  ...
  sudo docker run --name focusflow-app \
    ...
    -e "PORT=8000" \
    -e "DEBUG=true" \
    -it mubaraklouis/missservice:0.0.12 /bin/sh
else
  ...
  sudo docker run --name focusflow-app \
    ...
    -d mubaraklouis/missservice:0.0.12
...
echo "2. Try running with interactive shell: sudo docker run -it --rm mubaraklouis/missservice:0.0.12 /bin/sh"
