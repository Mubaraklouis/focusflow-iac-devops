...
sudo docker pull mubaraklouis/focusflow-cms-service:1.3.0
...
if [ "$debug_mode" = "y" ]; then
  ...
  sudo docker run --name focusflow-app \
    ...
    -e "PORT=8000" \
    -e "DEBUG=true" \
    -it mubaraklouis/focusflow-cms-service:1.3.0 /bin/sh
else
  ...
  sudo docker run --name focusflow-app \
    ...
    -d mubaraklouis/focusflow-cms-service:1.3.0
...
echo "2. Try running with interactive shell: sudo docker run -it --rm mubaraklouis/focusflow-cms-service:1.3.0 /bin/sh"
