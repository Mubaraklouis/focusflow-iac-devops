...
sudo docker pull mubaraklouis/miss-service:0.0.1
...
if [ "$debug_mode" = "y" ]; then
  ...
  sudo docker run --name focusflow-app \
    ...
    -e "PORT=8000" \
    -e "DEBUG=true" \
    -it mubaraklouis/miss-service:0.0.1 /bin/sh
else
  ...
  sudo docker run --name focusflow-app \
    ...
    -d mubaraklouis/miss-service:0.0.1
...
echo "2. Try running with interactive shell: sudo docker run -it --rm mubaraklouis/miss-service:0.0.1 /bin/sh"
