...
sudo docker pull mubaraklouis/focusflow:1.7.4
...
if [ "$debug_mode" = "y" ]; then
  ...
  sudo docker run --name focusflow-app \
    ...
    -e "PORT=8000" \
    -e "DEBUG=true" \
    -it mubaraklouis/focusflow:1.7.4 /bin/sh
else
  ...
  sudo docker run --name focusflow-app \
    ...
    -d mubaraklouis/focusflow:1.7.4
...
echo "2. Try running with interactive shell: sudo docker run -it --rm mubaraklouis/focusflow:1.7.4 /bin/sh"
